...
- **Comps grounding ($0 path):**
  ...
  - **HumbleWorth** — open-source valuation model **self-hosted via Docker/Cog** (free, CPU, ~2 GB RAM), returning an `auction/marketplace/brokerage` triple. (Hosted Replicate API ~$0.10/1k is the fallback.) ⚠️ A *model estimate*, trained through early-2024 — pair with NameBio's real stats, don't rely on it alone.
    - ❌ **CORRECTED 2026-07-16 — the triple is NOT "a modeled low/mid/high range"** (as this section previously claimed). `auction`/`marketplace`/`brokerage` are **three distinct sale channels** at the **P50 / P97.5 / P99.25** percentiles — not a low/mid/high band of one distribution. A Tier-2 prompt presenting them as a single range would invite the model to "reconcile" three numbers that were never in tension. Carried to 5c.
    - ❌ **The free hosted endpoint (`valuation.humbleworth.com`) is DEAD** (verified 2026-07-16 from two independent networks; HumbleWorth's own `/about/api` documents only the Replicate route). This **breaks ratified decision #3.2's HumbleWorth leg** ("hosted endpoint on Windows-local") — see the **2026-07-16 entry in `DECISIONS.md`** (⚠️ needs re-ratification) and [`PHASE-5A-DESIGN.md`](PHASE-5A-DESIGN.md). Interim: **Phase 5 ships NameBio-only**; `value_range` reserves `"modeled": null` so HumbleWorth lands later as a data change, not a migration.
  ...
  - ✅ **SPIKE DISCHARGED 2026-07-16 — the free path over-delivers; the hedge is NOT promoted for NameBio.** Confirmed live: `GET /retailstats-download` = **6.7 MB, 97,568 keywords, 1.8 s**, free/no-auth/no-signup (+ `/tldstats-download` = 741 TLDs); ToS **explicitly permits** free data in products/services **with attribution** (the ban is *Paid-API-only*). **Measured** limits: stats endpoints 4 req/60 s rolling *per-endpoint* (recovery ~64 s), **no `Retry-After`/`X-RateLimit-*` headers**; the `*-download` endpoints have an **independent, much longer window** (>30 min) ⇒ **a download 429 is not retryable in-run — the daily cron is the retry** (inverting Phase 4's policy). Full detail: [`PHASE-5A-DESIGN.md`](PHASE-5A-DESIGN.md). *The original concern is preserved below for the record.*
...
- ~~**NameBio free-tier empirical spike** — Phase-5 pre-task (per ratification #2), run before Phase-5 prompt design.~~ ✅ **DONE 2026-07-16** — free path over-delivers; hedge not promoted for NameBio ([§4.5](#45-phase-5--two-tier-ai-scoring), [`PHASE-5A-DESIGN.md`](PHASE-5A-DESIGN.md)).
- ~~HumbleWorth stays a **hosted endpoint on Windows-local**; **self-host via Docker on the VPS later**.~~ ❌ **FALSE as of 2026-07-16 — the free hosted endpoint is DEAD** (two independent networks; undocumented on HumbleWorth's own API page). **Breaks ratified #3.2's HumbleWorth leg → ⚠️ needs re-ratification**; see the 2026-07-16 entry in [`DECISIONS.md`](../DECISIONS.md). Interim (selected): **ship Phase 5 NameBio-only**, add HumbleWorth at VPS migration; Replicate (~$0.09/mo) is the easy upgrade. `value_range` reserves `"modeled": null` so it lands as a data change, not a migration.
...
- **HumbleWorth self-hosted on the VPS** *(owner request, f4).* ❌ **Amended 2026-07-16: the free hosted
  endpoint is DEAD**, so the "use the free hosted endpoint on Windows-local" plan is not executable — this is
  what breaks ratified #3.2's HumbleWorth leg (⚠️ needs re-ratification; see [`DECISIONS.md`](../DECISIONS.md)
  2026-07-16). **Interim (selected): Phase 5 ships NameBio-only and HumbleWorth arrives at VPS migration**, where
  **self-hosting the open-source model via Docker/Cog** gives free, unlimited, offline valuations. Replicate
  (~$0.10/1k ≈ $0.09/mo at our volume) remains the easy earlier upgrade if the modeled leg is wanted sooner.
