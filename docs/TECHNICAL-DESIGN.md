...
- **Comps grounding ($0 path):**
  ...
  - **HumbleWorth** — open-source valuation model **self-hosted via Docker/Cog** (free, CPU, ~2 GB RAM), returning an `auction/marketplace/brokerage` triple. (Hosted Replicate API ~$0.10/1k is the fallback.) ⚠️ A *model estimate*, trained through early-2024 — pair with NameBio's real stats, don't rely on it alone.
    ...
    - ❌ **The free hosted endpoint (`valuation.humbleworth.com`) is DEAD** (verified 2026-07-16 from two independent networks; HumbleWorth's own `/about/api` documents only the Replicate route). This broke ratified decision #3.2's HumbleWorth leg ("hosted endpoint on Windows-local"); ✅ **re-ratified 2026-07-17 → the `ValuationProvider` is implemented against Replicate at Phase 5c** (see the 2026-07-17 entry in `DECISIONS.md`). **Phase 5a ships NameBio-only** and `value_range` reserves `"modeled": null`, so 5c fills it as a data change, not a migration.
...
- ~~HumbleWorth stays a **hosted endpoint on Windows-local**; **self-host via Docker on the VPS later**.~~ ❌ **FALSE as of 2026-07-16 — the free hosted endpoint is DEAD** (two independent networks; undocumented on HumbleWorth's own API page). ✅ **Re-ratified 2026-07-17: the `ValuationProvider` is implemented against Replicate at Phase 5c** (~$0.09/mo; `REPLICATE_API_TOKEN`); see the 2026-07-17 entry in [`DECISIONS.md`](../DECISIONS.md). **Phase 5a ships NameBio-only** and `value_range` reserves `"modeled": null`, so 5c fills it as a data change, not a migration. Docker self-host stays the free VPS-phase alternative.
...
- **HumbleWorth modeled valuations** *(owner request, f4).* ❌ **Amended 2026-07-16: the free hosted endpoint
  is DEAD** (so the original "use the free hosted endpoint on Windows-local" plan is not executable). ✅
  **Re-ratified 2026-07-17: the `ValuationProvider` is implemented against Replicate at Phase 5c** (~$0.10/1k
  ≈ $0.09/mo at 30/day; `REPLICATE_API_TOKEN`), filling the `"modeled"` slot 5a reserves as `null`. See the
  2026-07-17 [`DECISIONS.md`](../DECISIONS.md) entry. **Docker/Cog self-host** (`r8.im/...`) stays the free,
  unlimited, offline VPS-phase path; later, optionally **fork/improve the model** — retrain past its
  early-2024 cutoff, or fine-tune on our own Phase-6 outcomes.
